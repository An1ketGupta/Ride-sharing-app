    ...
    return (
        <div className="min-h-screen flex items-center justify-center px-4 py-12 bg-gray-50 dark:bg-gray-900">
            <div className="w-full max-w-2xl">
                <div className="rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 shadow-lg p-6 sm:p-8 md:p-10">
                    {/* Header */}
                    <div className="text-center mb-8">
                        <div className="inline-flex items-center justify-center w-16 h-16 sm:w-20 sm:h-20 rounded-lg bg-primary mb-4">
                            <UserCircle className="w-8 h-8 sm:w-10 sm:h-10 text-white" />
                        </div>
                        <h2 className="text-3xl sm:text-4xl font-bold tracking-tight text-gray-900 dark:text-gray-100 mb-2">Create Account</h2>
                        <p className="text-gray-600 dark:text-gray-400 text-base sm:text-lg">Join thousands of riders today</p>
                    </div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-5">
                        ...
                        {(formData.user_type === 'driver' || formData.user_type === 'both') && (
                            <div className="space-y-4 p-4 rounded-lg bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700">
                                <div className="text-sm font-semibold">Driver Verification</div>
                                <div className="grid md:grid-cols-2 gap-4">
                                    {formData.documents.map((doc, idx) => (
                                        <div key={idx} className="space-y-2">
                                            <label className="text-sm font-semibold capitalize">{doc.doc_type} URL</label>
                                            <input
                                                type="url"
                                                value={doc.file_url}
                                                onChange={(e) => {
                                ...
                                <p className="text-xs text-gray-600 dark:text-gray-400">Your documents will be reviewed by an admin. You'll be available only after approval. You can add vehicles after registration.</p>
                            </div>
                        )}
                        ...
                        <div className="grid md:grid-cols-2 gap-5">
                            ...
                            <div className="space-y-2">
                                ...
                                <select 
                                    ...
                                    onChange={handleChange}
                                    className="w-full px-4 py-3 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all outline-none appearance-none cursor-pointer text-gray-900 dark:text-gray-100"
                                >
                        ...
                        <button
                            type="submit"
                            disabled={loading}
                            className="w-full py-3.5 bg-primary text-white font-semibold rounded-lg hover:bg-primary/90 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                        >
                            {loading ? (
                                <>
